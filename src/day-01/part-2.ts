const test = 'ggdone3nbmsthreefourninefiveoneightpr';
...
const letterNumbers: { [k: string]: number } = {
    'one': 1,
    'two': 2,
    'three': 3,
    'four': 4,
...
const getCalibrationOfLine = (line: string): number => {
    const match = [...line.matchAll(/\d|(?=(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine))/g)];
    const res = match.flatMap((m) => m.filter((v) => v));

    let first = res?.[0];
    let last = res?.[res.length - 1];
    if (first && last) {
        if (letterNumbers?.[first]) {
            first = letterNumbers[first].toString();
        }
        if (letterNumbers?.[last]) {
            last = letterNumbers[last].toString();
        }
...
const handler = (input: string) => {
    return input.split('\n')
        .map((line: string) => {
            return getCalibrationOfLine(line);
        }).reduce((acc: number, curr: number) => acc + curr, 0);
};

const res = handler(doc);
