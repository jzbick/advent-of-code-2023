...
const isGamePossible = (game: string): boolean => {
    const blueCubes = getAmountOfCubesForColorFromGame('blue', game);
    const redCubes = getAmountOfCubesForColorFromGame('red', game);
    const greenCubes = getAmountOfCubesForColorFromGame('green', game);

    return blueCubes <= cubes.blue && redCubes <= cubes.red && greenCubes <= cubes.green;
};
...
const handler = (input: string): number => {
    ...
    return games.filter(isGamePossible).reduce((acc, game) => {
        const gameId = game.match(/Game ((\d)|(\d\d)):/)?.[1] || 0;
