export const TEST_PUZZLE_INPUT = '467..114..\n'
    + '...*......\n'
    + '..35..633.\n'
    + '......#...\n'
    + '617*......\n'
...
const regExpPartNumbers = /(\d*)/g;
const regExpSymbols = /[!@#$%^&*()_+={}\[\]:;<>,?~\\\/-]/;

const getLines = (input: string): string[] => input.split('\n');

const isPartOfEngine = (num: string, lineInd: number, lines: string[]): boolean => {
    const indexOfNum = lines[lineInd].match('\\b' + num + '\\b')?.index;
    if (indexOfNum === undefined) {
        return false;
    }

    const from = Math.max(indexOfNum - 1, 0);
    const to = Math.min(indexOfNum + num.length, lines[lineInd].length - 1);

    const currLine = lines[lineInd];
    const currentLinePart = currLine.slice(from, to + 1);
    const isCurrentLinePart = regExpSymbols.test(currentLinePart);
    if (isCurrentLinePart) {
        return true;
    }

    const prevLine = lines[lineInd - 1];
    if (prevLine) {
        const prevLinePart = prevLine.slice(from, to + 1);
        const isPrevLinePart = regExpSymbols.test(prevLinePart);
        if (isPrevLinePart) {
            return true;
        }
    }
    ...
    if (nextLine) {
        const nextLinePart = nextLine.slice(from, to + 1);
        const isNextLinePart = regExpSymbols.test(nextLinePart);
        if (isNextLinePart) {
            return true;
        }
    }
...
const handler = (input: string): number => {
    ...
    return lines.flatMap((line, lineId, arr) => {
        const match = line.match(regExpPartNumbers)?.filter(Boolean);
        if (match) {
            let strings: string[] = match.filter((num) => {
                return isPartOfEngine(num, lineId, arr);
            });
            console.log(lineId, match.filter(s => !strings.includes(s)));
            return strings;
        }
        return [];
