export const TEST_PUZZLE_INPUT = ''
    + '467..114..\n'
    + '...*......\n'
    + '..35..633.\n'
    + '......#...\n'
...
const getNumberForIndex = (idx: number, line: string): number => {
    const nums = line.matchAll(/\d+/g);

    const number = [...nums].find((match) => {
        if (match.index !== undefined) {
            const matchIdx = match.index;
            const matchEnd = match.index + match[0].length;
            if (matchIdx <= idx && matchEnd >= idx) {
                return true;
            }
        }
        return false;
...
const getNumberIndices = (linePart: string, idx: number): number[] => {
    const nums = linePart.matchAll(/\d+/g);
    return [...nums].map((match) => {
        if (match.index !== undefined) {
            return match.index + idx - 1;
        }
        return 0;
    });
};

const getGearRatio = (idx: number, lineId: number, lines: string[]): number => {
    const line = lines[lineId];

    const prevLine = lines[lineId - 1] ?? EMPTY_LINE;
    const currLine = lines[lineId];
    const nextLine = lines[lineId + 1] ?? EMPTY_LINE;

    const prevLinePart = prevLine.slice(idx - 1, idx + 2);
    const currLinePart = currLine.slice(idx - 1, idx + 2);
    const nextLinePart = nextLine.slice(idx - 1, idx + 2);

    const prevLineNumbers = getNumberIndices(prevLinePart, idx).map((numIdx) => getNumberForIndex(numIdx, prevLine));
    const currLineNumbers = getNumberIndices(currLinePart, idx).map((numIdx) => getNumberForIndex(numIdx, currLine));
    const nextLineNumbers = getNumberIndices(nextLinePart, idx).map((numIdx) => getNumberForIndex(numIdx, nextLine));
...
const handler = (input: string): number => {
    ...
    return lines.flatMap((line, lineId, arr) => {
        const matchAll = line.matchAll(/[*]/g);
        if (matchAll) {
            return [...matchAll].map((match) => {
                if (match?.index !== undefined) {
                    return getGearRatio(match.index, lineId, arr);
                }
                return 0;
            });
        }
...
console.log(handler(PUZZLE_INPUT));
